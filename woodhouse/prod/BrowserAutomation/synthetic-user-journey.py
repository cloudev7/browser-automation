...
class SyntheticUserJourney(unittest.TestCase):
    ...
    def execute_step(self, action, stepSeq, stepSeqSub, stepName, uri, xpath="", addAttr="", tls=False):
        ...
        try:
            if action == "get":
                #logger.debug("In {0} {1}".format("get", __LINE__), extra=LOG_HEAD)
                if tls == True:
                ...
                #logger.debug("In {0} {1}".format("hover", __LINE__), extra=LOG_HEAD)
                with timeout(seconds=SYSTEM_SLA_REQUEST_TIME_THRESHOLD):
                ...
                #logger.debug("In {0} {1}".format("click", __LINE__), extra=LOG_HEAD)
                with timeout(seconds=SYSTEM_SLA_REQUEST_TIME_THRESHOLD):
                    driver.find_element_by_xpath(xpath).click()

            elif action == "lookup" and addAttr != "":
                #logger.debug("In {0} {1}".format("lookup", __LINE__), extra=LOG_HEAD)
                with timeout(seconds=SYSTEM_SLA_REQUEST_TIME_THRESHOLD):
                ...
                #logger.debug("In {0} {1}".format("clear", __LINE__), extra=LOG_HEAD)
                with timeout(seconds=SYSTEM_SLA_REQUEST_TIME_THRESHOLD):
                    driver.find_element_by_xpath(xpath).clear()

            elif action == "keyin" and addAttr != "":
                #logger.debug("In {0} {1}".format("keyin", __LINE__), extra=LOG_HEAD)
                with timeout(seconds=SYSTEM_SLA_REQUEST_TIME_THRESHOLD):
                    driver.find_element_by_xpath(xpath).send_keys(addAttr)

            elif action == "select" and addAttr != "":
                logger.debug("IN ACTIOM SELECT - {0}".format(str(addAttr)), extra=LOG_HEAD)
                #logger.debug("In {0} {1}".format("select", __LINE__), extra=LOG_HEAD)

                with timeout(seconds=SYSTEM_SLA_REQUEST_TIME_THRESHOLD):
                    logger.debug("BEFORE SELECT {0}".format(str(addAttr)), extra=LOG_HEAD)
                    select = Select(driver.find_element_by_id(xpath))
                    select.select_by_index(2)
                    logger.debug("AFTER SELECT SUCCESSFUL", extra=LOG_HEAD)
